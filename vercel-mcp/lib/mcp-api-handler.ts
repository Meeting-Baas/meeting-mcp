...
function createFakeIncomingMessage(options: FakeIncomingMessageOptions = {}): IncomingMessage {
  ...
  if (body) {
    // We'll emit the data events on the next tick to simulate reading
    process.nextTick(() => {
    ...
    // Empty body, just end the stream
    process.nextTick(() => {
      req.emit('end');
    });
  }
...
export function initializeMcpApiHandler(
  ...
  return async function mcpApiHandler(req: IncomingMessage, res: ServerResponse) {
    ...
    if (url.pathname === '/sse') {
      ...
      const logInContext = (severity: 'log' | 'error', ...messages: string[]) => {
        logs.push({
          type: severity,
          messages,
        });
      ...
      console.log(`Published request to ${sessionId}`);
